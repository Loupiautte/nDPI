 ...
 * system for remotely installing, configuring && maintained Windows Servers.
 *
...
static inline bool match_bmdp_payload(uint32_t payload, uint32_t len) {
        ...
        /* Byte 3 is always Xd, where X >=0 && X < 8 */
        if ((ntohl(payload) & 0x7d00) != byte3)
                return false;

        /* There seem to be a fixed set of values for byte 4 && these seem
