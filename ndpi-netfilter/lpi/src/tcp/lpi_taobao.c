 ...
 * CDN hosts && TCP port 80, but it is clearly not HTTP. Users doing this
 * protocol also speak conventional HTTP && HTTPS to the same Taobao host
 * at the same time, so it isn't a substitute for HTTP. Possibly a streaming
 * media protocol?
 *
 * For now I'm just going to use the generic name 'Taobao' && hope that
